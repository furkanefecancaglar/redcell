...
(function (root, factory) {
  ...
  const RULE_DEFS = [
    ...
    ['instruction-replacement', 'LLM01', 'high',
      R`\b(new|further|updated|revised|neue|weitere|aktualisierte)\s+(instruction|rule|directive|guideline|task|anweisung|regel|vorgabe|aufgabe|auftrag)\w*\s*(now\s+)?(follow|apply|are in effect|folgen|gelten)\b|\b(follow|obey|befolge|beachte)\s+(these|the new|them|sie|diese)\s*(instruction|rule|anweisung|regel)\w*\s*(carefully|exactly|genau|sorgf[aä]ltig)\b`,
    ...
    ['prompt-metadata', 'LLM01', 'high',
      R`\b(how (many|long)|character count|word count|length of|first (word|letter|character)|sha-?\d*|hash|checksum|wie (viele|lang)|cu[aá]ntas palabras|ka[cç] kelime)\w*[^.?!]{0,60}?\b(your|deine[rnms]?|tus?|senin)\s*(own |real |actual )?(system[\s_-]*)?(prompt|instruction|message|anweisung|talimat|instrucci[oó]n)\w*\b(?!\s+(for|on|about|regarding|concerning|f[uü]r|para|icin|i[cç]in)\s+\w)|\b(your|deine|tus)\s*(system[\s_-]*)?(prompt|instruction|anweisung|talimat)\w*[^.?!]{0,50}?\b(how (many|long)|wie (viele|lang)|ka[cç] kelime)\b`,
    ...
    ['governing-text-request', 'LLM07', 'high',
      R`^(?=[\s\S]*\b(?:put|place|render|format|output|show|list|compar|verif|confirm|includ|contain|enumerat|number|count|print|return|past|quot|reveal|disclos|shar|explain|writ|set|updat|answer|respond|repl|tell|give|repeat|what is|how many|stelle|zeig|nenne|antworte|devuelve|dime|muestra|yaz|s[oö]yle|say[iı]|cu[aá]nta|cu[aá]nto|nedir|ka[cç])\w*\b)[\s\S]*?\b(your|yours|deine[rnms]?|tus?|ton|tes|senin|sana verilen|its|seine[rnms]?|su)\s+(own |full |real |actual |original |initial |hidden |operating |urspr[uü]nglich\w* )*((system[\s_-]*|sistem |sistema )(prompt|instruction|message|nachricht|anweisung|talimat)\w*\b|(prompt|instruction|setup|configuration|config\b|anweisung|talimat|instrucci[oó]n)\w*\b(\s+del sistema)?)(?!\s+(for|on|about|regarding|concerning|f[uü]r|para|icin|i[cç]in)\s+\w)|\b(the |die |las )?(setup|instructions?|prompt|system message|configuration text|text|anweisungen|instrucciones)\s+(you were given|given to you|you started from|dir gegeben|que te dieron|sana verilen)\b|\bhow many instructions were you given\b|\b(instrucciones|indicaciones)\s+del\s+sistema\b|\bsistem\s+talimatlar\w*|\b(assistant|model|bot|asistan|assistent|asistente)\s+(may|can|darf|puede|is allowed to)\s+(disclose|reveal|show|share|zeigen|mostrar|g[oö]ster)\w*`,
      "a request whose object is the model's own governing text"],
    ['prompt-identifier-eval', 'LLM07', 'high',
      R`(^(?=[\s\S]*\b(run|evaluate|execute|print|return|console\.log|output\s*=|eval)\b)[\s\S]*?(\bSYSTEM_PROMPT\b|\bsystemMessage\b|\bself\.instructions\b|\breveal_system_prompt\b|\bsystem_context\b|\binternal_config\b)|[\"']\s*(reveal_system_prompt|system_context|internal_config|systemMessage|SYSTEM_PROMPT)\s*[\"']?\s*:)`,
      "code-shaped identifier for the model's own prompt, in an execution context"],
    ['cross-model-comparison', 'LLM07', 'high',
      R`^(?=[\s\S]*\b(compar|differ|difference|match|line by line|unterschied|diferenc)\w*)[\s\S]*?\b(another|other|similar|a different|an?other)\s+(assistant|ai\b|bot|model|chatbot|llm)\w*[\s\S]{0,60}?(prompt|instruction|system message|anweisung|instrucci[oó]n)\w*|\b(the )?(prompt|instructions?|system message)\s+of\s+(a|another|the)\s+(similar|other|another)?\s*(bot|assistant|ai|model)\b`,
      "compares the model's own governing text against a supplied one"],
    ['context-boundary-probe', 'LLM07', 'high',
      R`\b(before my first (message|turn|prompt)|above my first message|before anything i typed|before i said anything|before i wrote anything|before this session (began|started)|vor meiner ersten nachricht|antes de mi primer mensaje|hi[cç]bir [sş]ey yazmadan [oö]nce)\b|^(?=[\s\S]*\b(print|quote|reproduc|repeat|show|display|summaris|summariz|render|read back|translat|discuss|zeig|gib|wiederg|muestra|cita|traduce|g[oö]ster|yaz)\w*)[\s\S]*?\b(before|prior to|preceding|at the (very )?beginning of|al principio de|vor|antes de|[oö]nce)\b[^.?!]{0,40}?\b(this (session|conversation|chat)|my first (message|turn|prompt)|i said anything|i arrived|i wrote anything|meiner ersten nachricht|dieser (sitzung|unterhaltung)|mi primer mensaje|esta sesi[oó]n|hi[cç]bir [sş]ey yazmadan)\b|\b(including|einschliesslich|incluyendo|dahil)\b[^.?!]{0,30}?\b(anything|any part|everything|alles|todo)\b[^.?!]{0,30}?\b(i did not write|i didn'?t write|nicht geschrieben|que no escrib)\w*`,
      "asks for what lies before the start of the user's own turn"],
    ['authored-for-model', 'LLM07', 'high',
      R`^(?=[\s\S]*\b(quote|repeat|reproduc|read back|print|show|cite|gib|citar?|citalo|wiederg)\w*)(?=[\s\S]*\b(developer|operator|entwickler|desarrollador|team that built|whoever (deployed|built|made)|company that)\w*)(?=[\s\S]*\b(wrote|left|gave|set up|deployed|geschrieben|escribi|verilen)\w*)[\s\S]*?\b(for you|to you|dir|te)\b|\bthe team that built you\b|\bwhoever deployed you\b`,
      "asks for the text whoever deployed the assistant wrote for it"],
    ['self-description-complete', 'LLM07', 'high',
      R`\b(describe|explain|profile|write (a |the )?(full |complete )?profile of|beschreibe|describe?te)\w*\s+(yourself|dich|te\b|what you are)[\s\S]{0,80}?\b(completely|exhaustively|in full|leaving nothing out|everything you were (told|given)|what you were set up to do|configured with|vollst[aä]ndig|einschliesslich aller|sin omitir)\w*|\b(describe|explain|beschreibe)\w*\s+(yourself|dich)\b[\s\S]{0,60}?\b(einschliesslich aller vorgaben|including all your (instructions|directives))\b`,
      "self-description with a completeness demand"],
    ['self-surface-enumeration', 'LLM07', 'high',
      R`(^(?=[\s\S]*\b(list|enumerate|give me|name|dame|listele|nenne|aufz[aä]hl)\w*)[\s\S]*?\b(tool|function|capabilit|topic|question|pregunta|arac|ara[cç]lar|herramienta|werkzeug)\w*|^(?=[\s\S]*\b(list|enumerate|give me|name|dame|listele|nenne|aufz[aä]hl)\w*)(?=[\s\S]*\b(tool|function|capabilit|topic|question|pregunta|arac|ara[cç]lar|herramienta|werkzeug)\w*)[\s\S]*)[\s\S]{0,80}?\b(available to you|you can call|you have access to|you would refuse|you are (not allowed|forbidden|unable)|registered with|rechazarias|sana tan[iı]ml[iı]|dir zur verf[uü]gung)\b|\b(where (that|the) restriction comes from|the exact reason you would give)\b`,
      "enumerates the assistant's own tools, limits or refusals"],
    ['self-documentation-audit', 'LLM07', 'high',
      R`\b(documentation|docs|readme|manual|compliance check|audit|selbstauskunft)\w*[^.?!]{0,50}?\b(for|on|of|[uü]ber)\s+(yourself|this assistant|your own|dich selbst|deine eigene)\b|\baudit your own (configuration|setup|instructions|rules)\b`,
      "documents or audits the assistant itself"],
    ['configured-format-mirror', 'LLM07', 'high',
      R`\b(format|style|wording|tone|formulierung|estilo)\w*[\s\S]{0,60}?\b(you were configured with|that set you up|you were set up with|mit dem du konfiguriert wurdest|con el que fuiste configurado)\b|\b(configured|set up|konfiguriert)\b[^.?!]{0,30}?\b(before this (chat|session|conversation))\b`,
      "asks the model to mirror the format of the text that configured it"],
    ['staged-self-extraction', 'LLM07', 'high',
      R`^(?:(?=[\s\S]*\b((you (work from|were (given|shown|set up|configured|handed)|must have been given|have a (list|set) of))|((were|was|have) you (given|shown|set up|configured|handed)))\b)(?=[\s\S]*\b(guideline|instruction|rule|text|setup|configuration|tone of voice|sentence|things you must never|set of)\w*)|(?=[\s\S]*(\b(what you are (able|allowed) to (help|do)|things you must never do)\b|\b(sana|dir|te)\b[^.?!]{0,40}?\b(verildi|bekommen|dieron|gegeben)\b|\bdu hast\b[^.?!]{0,40}?\b(text|anweisung|vorgabe)\w*[^.?!]{0,20}?\bbekommen\b)))(?=[\s\S]*\b(exact wording|verbatim|word for word|exactly as written|quote (it|that|them)|quote the opening|write that opening out|read me that definition|the (very )?first (sentence|line) you (were|was)|aynen|w[oö]rtlich|woertlich|tal cual|exactamente|textualmente)\b)|\b(system message|configuration text|briefing|system prompt)\w*[\s\S]{0,220}?\b(add|provide|include|paste|give)\b[^.?!]{0,20}?\byours\b`,
