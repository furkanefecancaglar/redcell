...
> - `POST /firewall` — a runtime injection firewall (regex, 59 detectors, 4 languages, 0 API, ~60us of compute, ~3ms added over a static response). Client-side JS + Python ports too.
...
>
> - **Firewall**: inspects untrusted input for injection/jailbreak/exfil (59 detectors, 4 languages, pure pattern-match, 0 API, ~60us of compute, ~3ms added over a static response). Python + JS + edge API.
