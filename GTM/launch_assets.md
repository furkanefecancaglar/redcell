...
- ReDoS-audited (bounded quantifiers, 16 KB cap). Python↔JS engines byte-for-byte identical, enforced by an automated parity test; 176 tests incl. fuzz + self-regression corpus. SARIF export for code-scanning (/r/<id>.sarif). 0-API, edge-hosted, vendorable, 4-tool MCP server (firewall_check, scan_prompt, tool_check, agent_check).
