...
- Tool-call firewall (/toolcheck): assesses {name, arguments} for destructive names, privilege escalation, exec/shell, SSRF/local-file, secret-exfil, unbounded financial transfers — probed to 0 FP on a benign tool-call corpus.
- Unified /agentcheck runs all three surfaces in one call.
- ReDoS-audited (bounded quantifiers, 16 KB cap). Python↔JS engines byte-for-byte identical, enforced by an automated parity test; 154 tests incl. fuzz + self-regression corpus. SARIF export for code-scanning. 0-API, edge-hosted, vendorable, MCP.
...
3/ It also firewalls the input (34 detectors + deobfuscation: base64/leetspeak/homoglyph/zero-width/invisible-unicode, + optional semantic for paraphrases) and scores your system prompt (22 checks + a hardened-prompt kit).
